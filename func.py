...
# Define a sample function that the agent can call
def calculator_tool(expression: str) -> str:
    try:
        # Warning: use with trusted input only (or sandbox!)
...
tools = [
    Tool(
        ...
        description="Answers questions from PDF knowledge base or falls back to Web Search."
    ),
    Tool(
        name="Calculator",
...
agent = initialize_agent(
    ...
    llm=ChatOpenAI(
        ...
        openai_api_key=os.getenv("OPENROUTER_API_KEY"),
        model="gpt-3.5-turbo",
        function_call="auto"  # This tells the model to use function calling
    ),
